    ...
    list.onKey = function(evt) {
      if( evt.type == Event.KEY_DOWN )
      {
        switch(evt.keyCode)
        {
          ...
          case Event.VK_END:   setCurrent(list.length - 1); return true;
          case Event.VK_UP:    setCurrent(Integer.max(0,list.currentIndex - 1)); return true;
          case Event.VK_DOWN:  setCurrent(list.currentIndex + 1); return true;
          case Event.VK_PRIOR: setCurrent(Integer.max(0,list.currentIndex - visible_items + 1)); return true;
          case Event.VK_NEXT:  setCurrent(list.currentIndex + visible_items - 1); return true;
        }
      } 
    }
...
class VGrid : Behavior 
{
  function attached() { 
    ...
    VirtualList { container: this.tbody, setupItemView: setupItemView  };
  }
