...
function filter_by_display(server) {
  try {
    ...
    console.error("filter_by_display");
...
function load_news_feed() {
  try {
    ...
    console.error("load_news_feed");
    console.error(printf("Caught exception: %s\n%V", e, e.stacktrace));;
  }
}

function image_callback(image2) {
  try {
    ...
    if (image2) {
      ...
      console.error(image);
      console.error(image.prototype);
      console.error(image.packaging);
      console.error(image.width);
      console.error(image.height);
      if (image && this.url) {
        ...
        try {
          ...
          console.error("image_callback.toBytes");
          console.error(printf("Caught exception: %s\n%V", e, e.stacktrace));
        }
        try {
          base64 = sciter.toBase64(bytes);
        } catch (e) {
          console.error("image_callback.base64");
          console.error(printf("Caught exception: %s\n%V", e, e.stacktrace));
        }
        try {
          news_items[this.id].html = news_items[this.id].html.replace(url_regex, "data:image/webp;base64," + base64);
        } catch (e) {
          console.error("image_callback.replace");
          console.error(printf("Caught exception: %s\n%V", e, e.stacktrace));
        }
      } else {
...
const variable_observer = {
  set: function (target, prop, receiver) {
    try {
      ...
      console.error("variable_observer");
      console.error(printf("Caught exception: %s\n%V", e, e.stacktrace));;
    }
  }
}
...
function initialize_variables() {
  try {
    ...
    console.error("initialize_variables");
    console.error(printf("Caught exception: %s\n%V", e, e.stacktrace));;
  }
}
...
function getServersCallback(results) {
  ...
  */
  try {
    ...
    console.error("getServersCallback");
    console.error(printf("Caught exception: %s\n%V", e, e.stacktrace));;
  }
}
...
function launcher_progress(progress) {
  try {
    ...
    console.error("launcher_progress");
    console.error(printf("Caught exception: %s\n%V", e, e.stacktrace));;
  }
}
...
function check_launcher_result(new_version = undefined) {
  try {
    ...
    console.error("check_launcher_result");
    console.error(printf("Caught exception: %s\n%V", e, e.stacktrace));;
  }
}
...
function onUpdateCallback(reason) {
  try {
    ...
    console.error("onUpdateCallback");
    console.error(printf("Caught exception: %s\n%V", e, e.stacktrace));;
  }
}
